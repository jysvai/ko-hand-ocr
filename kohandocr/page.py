...
# 줄 높이의 아래 한계(쪽 높이 대비).
#
# 2026-09-20 에 0.012 에서 0.018 로 올렸다. **칸 수로 골랐다** — hand-01 은
# 정답이 세 줄인데 0.012 에서 칸이 여섯 개로 잘리고 셋이 쓰레기였다. 0.018 이면
# 넷이 되고, 나머지 열한 장은 칸 수가 하나도 안 바뀐다.
#
# 다만 **점수는 안 올랐다.** 조합 셋(v55-2500+v61-2500+v62 / v18-5000+v20b+
# v19-17500 / v67-5000+v62+v55-2500)으로 재 보니 열한 장이 모두 소수점까지
# 같았다. 쓰레기 칸은 `pair()` 가 짝을 못 지어 버리므로 점수를 깎고 있지
# 않았다. 올린 까닭은 읽지 않아도 되는 칸을 덜 읽는 것과, 앱에서 사람이 검토할
# 자리에 헛줄이 덜 나가는 것이다. **hand-01 의 94.4% 는 자르기가 아니라 읽기다.**
_MIN_LINE = 0.018
