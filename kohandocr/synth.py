...
# (실측: 다섯 글자 부서 이름이 여섯 글자로, 세 글자 이름이 네 글자로 늘어
# 읽혔다 — 두 줄 다 없는 글자가 하나씩 끼어들었다).
#
...
#
# hand-06 에 둘이 같이 있다. 칸2 부서 이름 앞에는 지운 덩이가 있고(모델이 그걸
# 'Team' 으로 읽었다), 칸0 서약 제목의 한 글자는 덧그어 진해져 있다(모델이
...
def render(text: str, fonts: Fonts, rng: random.Random,
    ...
    if len(text) >= 2 and rng.random() < (STRIKE if strike is None else strike):
        ...
        # hand-06 의 부서 이름 앞 덩이도, hand-01 의 글자 위 자국도 한 글자
